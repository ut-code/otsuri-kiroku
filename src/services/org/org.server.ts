...
export async function createOrg(input: { name: string; image?: string }) {
  const session = await requireSession();
  const createdAt = now();
...
export async function deleteOrg(input: { orgId: string; confirmName: string }) {
  ...
  if (memberCount > 1)
    throw new Error("Cannot delete an organization with multiple members");
...
export async function acceptInvite(input: { token: string }) {
  ...
  const existing = await prisma.organizationMember.findFirst({
    where: { orgId: invite.orgId, userId: session.user.id },
  });
  ...
  setActiveOrgIdCookie(invite.orgId);
  return { ok: true } as const;
}
